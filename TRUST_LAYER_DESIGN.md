...
状態: **Phase A / B / 失効 / 計測整合性 実装済み（2026-08-19）**。真のmeter attestation（物理計測）は未着手
日付: 2026-08-18
...
## 18. Phase C の下限 — 計測の自己整合（2026-08-19）

真のmeter attestationは「このマシンが本当にその計測器で測ったか」を第三者が保証すること。
**EDENが所有していないハードウェアが要る**（§0で最初に宣言した通り）。
だがその下に、ソフトウェアだけで敷ける床がある。

### 18.1 レシートは自分の計測主張を再導出できる

```text
estimated  : energy == cpu_seconds × watts_per_cpu_second_assumed
os-counter : energy == (mean_active_mw − mean_idle_mw)/1000 × wall_seconds
```

`eden attest` が全レシートでこれを再計算する。実台帳149件はすべて coherent。

### 18.2 検査の射程を実測した（これが肝）

| 攻撃 | 結果 |
|---|---|
| エネルギーだけ1/100に詐称 | incoherent |
| wall時間を10倍に伸ばす | incoherent |
| アイドル電力を実測超に詐称 | incoherent（active < idle） |
| 負のエネルギー | incoherent |
| **profileとenergyを同時に半分へ（整合させたまま嘘）** | **coherent — 通る** |

最後の1行がこの層の正直な限界であり、**テストに固定した**。
雑な嘘は弾けるが、整合した嘘は弾けない。それを弾くには EDEN が持っていない計測器が要る。
だから名前を attestation ではなく **coherence** にした — 主張が自分自身と整合すること以上を
名乗らせない。

### 18.3 経済ゲートへ接続

`incoherent` は前線に参加できない。**自分の profile と矛盾する測定は、誰が署名していても測定ではない。**

### 18.4 副産物 — 監査のための値を丸めていた

`mean_active_mw` / `mean_idle_mw` を小数1桁に丸めて保存していたため、
**保存値からの厳密な再導出ができなかった**（実測: 70件が丸め由来の微差で不一致）。
今後は丸めない。既存レシートは丸め幅を見込んだ許容範囲で判定する（過去は変えない — 憲法IV）。
「監査可能性のために残す値を、表示の都合で丸めてはならない」という規則を得た。


---

## 19. 乱数を「先に約束し、後で開く」（2026-08-19）

§15までの実装は「commitmentを永続化 → 乱数を取得」で、operatorが乱数を見てから
epochを放棄する経路はPENDINGスタブとして*可視*になっただけだった。
監査が挙げた完全解 —— **未来のdrand roundを事前に固定する** —— を実装した。

### 19.1 なぜこれで選択の余地が消えるか

drandは30秒周期の公開ビーコンで、round番号と時刻が決定的に対応する。
**まだ到来していないroundの値は存在しない**（実測: 404）。
だからepoch開設時に「round 6389314を使う」と宣言してコミットすれば、
**operator自身にも値が見えない**。見えないものは選べない。

```text
Phase A: 未来のround番号を決める → 参加者コミットと共に台帳へ永続化
Phase B: そのroundが到来するまで待つ → 開封 → seed確定
```

### 19.2 第三者が検証できる（実測）

```text
epoch 84feb00fa8513966
randomness: drand:8990e7a9aaed2ffe:round-6389314
独立にdrandから取得した値と台帳の値が一致 : True
再導出したseedが台帳と一致               : True
```

**誰でも同じ値を引いて同じseedを再計算できる。** epochの公正さは
EDENを信用しなくても確かめられるようになった。

### 19.3 実装中に見つけた1周期のずれ

最初の実epochは `promised round unavailable — fallback` で落ちた。
原因は時刻式が `genesis + (round-1)×period` だったこと。
実測すると正しくは **`genesis + round×period`** で、待機が常に30秒足りなかった。
**約束した乱数が使われず、静かに弱い乱数へ落ちていた** — 実epochを開くまで
表に出ない種類の欠陥。公開の遅延にも耐えるようリトライも入れた。

### 19.4 残る限界（正直に）

- drandが到達不能なら fallback（operator-local urandom）へ落ち、
  **その事実がrandomness_sourceに明記される**。黙って弱くならない
- operatorは依然としてepochを放棄できる。ただし放棄してもPENDINGスタブが残り、
  かつ次のepochの乱数も予測できないので、**grindingに利益がない**構造になった


---

## 20. 値付けは毎回導出する — cacheに決めさせない（2026-08-19）

第8次監査の2点。どちらも**実際に攻撃が通ることを確認してから**直した。

### 20.1 攻撃1: 未知メーターを名乗れば検算を回避できた

`_admits_to_frontier` は `incoherent` だけを拒否し、`unknown` を通していた。
実測: `method: "oracle-9000"` を名乗り、3.0Jのはずのrunを **0.0001J** と主張したレシートが
**前線に載った**。検算できないメーターを名乗るのが、最も安い侵入経路だった。

修正: **`coherent` 以外は値付けしない**。検証不能な主張が、検証可能な主張より
価値を持ってはならない。

### 20.2 攻撃2: cache列に嘘を書けば矛盾した測定が通った

`group_stats` は `r["trust_state"] or trust_state_of(...)` と書かれており、
**DBに値があればそれを使っていた**。実測: 本体は 0.0002J vs profile 3.0J で明白に矛盾する
レシートに、`trust_state='SIGNED'`, `meter_coherence='coherent'` と書き込むだけで
**前線に載った**。

修正: 値付け時は署名・失効・整合性を**毎回レシート本文から導出する**。
DB列は表示用cacheに降格。**値を決めるcacheは、嘘を書く場所になる。**

```text
stored columns  → 表示のみ
fresh derivation → signature / revocation / coherence を毎回再計算
                 → admission
```

### 20.3 テストのフィクスチャが現実より緩かった

修正後、テストが10件落ちた。原因は**テスト用レシートが `measurement_profile` を持たない
簡略形**だったこと — 実レシート149件はすべて持っている。
新しいゲートが、テストデータの非現実性を暴いた形。フィクスチャを実レシートの形に揃えた。

さらに `INVALID` を検証するテストが **cache列に "INVALID" と書くだけ**で成立していた。
fresh導出になった今、それは何も証明しない。**本物の壊れた署名**（署名後にエネルギーを
書き換えたレシート）を作る形に直した。

### 20.4 残る改善余地（監査の指摘、未着手）

bootstrapは単純なrun resamplingで、instance難易度の非IID性を扱っていない。
paired/stratified bootstrapへ進む余地がある — 記録して次へ回す。

---

## 21. 比較の単位はrunではなくinstance（2026-08-19）

§20.4で「次へ回す」と書いた指摘への解答。実測してから設計を決めた。

### 21.1 epochが発行したのはinstanceであり、runではない

難易度はinstanceに属する。難しいinstanceは両方のrunnerを落とす。
それをrun単位で resample すると:

- **同一instanceの繰り返しが独立な観測に見える** → 区間が不当に狭まる（過剰発行の向き）
- **共有された難易度が各runnerに二重計上される** → 区間が不当に広がる（過小発行の向き）

修正: resample の単位を instance にした（cluster bootstrap）。
1 instance = 1 run の現データでは数値は完全に一致する。将来 repeat が入った時に
初めて差が出る予防的修正であり、**今のデータで何かが変わったとは主張しない**。

### 21.2 同一epochの比較はpairedでなければならない

epochは全runnerに同じinstanceを発行する。だから比較は対応のある標本であり、
instance集合を一度だけ引いて両者に同じ引きを見せれば、epochに属する変動が消える。

実測（本番台帳、3 epoch）:

```text
                                unpaired intervals      paired difference
1.5b vs 7b  (token bugs)     [55,162] vs [213,1060]    Δ [-979, -118]  両方が認定
1.5b vs 7b  (semantic bugs)  [62,435] vs [300, inf]    Δ [-inf,  -52]  pairedのみ
```

成功率の軸も同型の欠陥を持つ。Wilsonは対応のない区間であり、11/12 対 6/12 を
分離できない。instance単位で見れば phi4 は cascade が落とした5問を解き、
cascade が解いた問題を一つも落としていない（McNemar exact p=0.031）。
**離散的な二値対応にはbootstrapより正確な道具（厳密二項尾）がある**ので、
成功率軸はMcNemar、エネルギー軸はpaired bootstrapとした。

### 21.3 これは「基準を緩めた」のか

pairedの方が認定は通りやすい。だからこの変更は**発行を増やす向き**であり、
そのまま黙って入れてよい種類の変更ではない。主張はこうである:

> unpairedは設計として保守的だったのではなく、共通instance比較に対して
> **単に道具が違っていた**。偶然厳しいことは安全性ではない。

代わりに、pairingが成立する条件を厳格に守る:

1. 同一epochであること
2. 全観測がinstance keyを持つこと — **配列の位置は同一性ではない**
3. 両者のinstance集合が完全に一致すること

条件2は実装中に自分のテストが捕まえた。instance keyが無い観測列に対して
`#0..#11` という位置由来のキーを振っており、**配列の並び順を「同じ問題」と
見なしてペアリングしていた**。存在しない構造の上で区間を締める行為であり、
このモジュールが禁じているものそのものだった。

### 21.4 発行を1件取り消した — cacheではなく到達経路の問題

同じ調査で、より重い欠陥が出た。

```text
記録保持者 16c4b89c:codefix_llm   1成功/6回   894 J/success   ci95 [288, inf]
挑戦者     cc846674:codefix_llm   5成功/12回  392 J/success   ci95 [213, 1060]
```

区間は完全に重なっている。それでも旧コードは**+502 CREDITを発行していた**。
理由: bootstrap区間は証明書生成時に計算されるが**保存されない**。
同一epoch内の比較はin-memoryの証明書同士なので区間が生きているが、
**前のepochの記録保持者はDBから読み戻され、区間を失って到着する**。
残るのは点推定への20%固定margin — それが通していた。

修正は§20の原則をそのまま適用した。保存もbackfillもしない。
**比較の時点で台帳から再導出する**。epochが発行したinstanceは既に書かれた
事実であり、その脇に置いた二つ目の写しは、二つが食い違える二つ目の場所になる。

台帳のコピーで3 epochを再生し、旧規則3 mint / 新規則4 mintを差分で確認した:
上の1件が消え、pairedによる2件が増えた。**本番台帳は書き換えていない。**
既存6件のnoteは `SIMULATED-DIST`（基準名なし）＝H4ゲート導入前の規則で
発行された履歴であり、憲法IVにより事実として残す。

### 21.5 副産物で見つけた2件

- `existing.append(cert)` が `if commit:` の内側にあり、**分析実行が
  commit実行より弱い規則をpreviewしていた**。dry runを根拠に読んでいたのは
  私自身なので、これは測定の土台の問題。commit外へ出した。
- 証明書を畳み込む集約クエリに `ORDER BY` が無かった。証明書は一枚ずつ前線へ
  入るので、畳み込む順序がどれが記録に挑むかを決める。
  「mintはledger順の純関数」と宣言している以上、順序をプランナ任せにはできない。
  `ORDER BY MIN(started_at), runner_id, code_hash` を明示した。

### 21.6 残る限界（正直に）

- pairedはepoch内でしか使えない。**epochを跨ぐ記録更新は依然unpaired**であり、
  そこでは共有難易度が消せない。epoch間で instance を共有する設計（共通holdout）
  が要るが、それは challenge sampling の再設計であり別課題。
- McNemarは1 instance = 1 runの時だけ適用する。repeatがある場合は
  paired verdictを出さずWilsonへ落とす（緩い方ではなく厳しい方へ落ちる）。
- bootstrap percentile区間はn=12では粗い。BCaや厳密法は未検討。

### 21.7 不変条件をfuzzして1件出た

発行ゲートに直接効く変更なので単体テストで終えず、乱数入力400件で不変条件を叩いた:
自己支配なし、相互支配なし、自分自身との差が0を含む、観測の並び順に依存しない、
McNemarが両方向で認定しない、全instanceで安い双子に負けない、そして差の反対称性。

反対称性だけが破れた。`int(0.025*(N-1))` と `int(0.975*(N-1))` は鏡像インデックスに
ならず、区間が1つ分だけ中心からずれる。**ずれる向きは認定が通る方**だった。
判定が「どちらのrunnerを先に書いたか」に依存してはならないので、両端を各端から
同じ距離の順序統計量に揃えた（`k=int(0.025*N)`、上端は `N-1-k`）。

差は1順序統計量ぶん（例: Δ上限 -523.5 → -522.8）で、台帳の再生結果は4 mintのまま
変わらない。それでも直したのは、大きさの問題ではなく**発行規則が対称でないこと自体**が
欠陥だから。
